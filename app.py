...
def send_email(rating, feedback):
    ...
    email_password = st.secrets["moop equm czqc ljqm"]   # Replace with st.secrets["EMAIL_PASSWORD"] for deployment
...
def main():
    ...
    st.write("Upload a document (up to 15,000 words) or paste text (up to 2,000 words) to generate summaries, highlighted keywords, and audio output.")
    
    # Note about warnings
    st.markdown("**Note**: If you see warnings about symlinks or `hf_xet` in the terminal, they are harmless. To resolve symlinks, enable Windows Developer Mode. For faster model downloads, run `pip install hf_xet`.")
    ...
    if input_type == "Upload File":
        uploaded_file = st.file_uploader("Upload Document (PDF, TXT, DOCX)", type=["pdf", "txt", "docx"], key="file_uploader")
        if uploaded_file:
        ...
        if st.session_state.uploaded_file:
            ...
            with st.spinner("Extracting text..."):
                ...
                if word_count > 15000:
                    st.error(f"Document exceeds 15,000 words (current: {word_count}). Please use a shorter document.")
                    return
                st.session_state.input_text = text
                st.info(f"Document word count: {word_count}")
        else:
        ...
        if st.button("Summarize Paragraph"):
            summarizer = load_summarizer()
            placeholder = st.empty()
            st.session_state.summary = None
            st.session_state.img_buffer = None
    ...
    st.error("Input Limits: Pasted text ≤ 2,000 words; documents ≤ 15,000 words.", icon="🚨")
